...
const statusMap = {
  pending: { label: "\u5f85\u5ba1\u6838", className: "bg-amber-50 text-amber-700 ring-amber-200", message: "\u4f60\u7684\u63a8\u8350\u5df2\u63d0\u4ea4\uff0c\u7ba1\u7406\u5458\u6b63\u5728\u5ba1\u6838\u3002" },
  approved: { label: "\u5df2\u901a\u8fc7", className: "bg-emerald-50 text-emerald-700 ring-emerald-200", message: "\u5ba1\u6838\u5df2\u901a\u8fc7\uff0c\u5730\u70b9\u5df2\u53d1\u5e03\u5230\u76ee\u7684\u5730\u5217\u8868\u3002" },
  rejected: { label: "\u672a\u901a\u8fc7", className: "bg-rose-50 text-rose-700 ring-rose-200", message: "\u8fd9\u6761\u63a8\u8350\u6682\u672a\u901a\u8fc7\u5ba1\u6838\uff0c\u8bf7\u67e5\u770b\u7ba1\u7406\u5458\u53cd\u9988\u3002" }
} as const;
  ...
  function renderCard(item: SpotSubmission, deleted = false) {
    ...
    return (
      <article key={item.id} className={`rounded-xl border bg-white p-5 shadow-sm ${deleted ? "border-slate-200 opacity-80" : "border-slate-200"}`}>
        ...
        <p className="mt-3 text-sm text-slate-700">{deleted ? `\u8fd9\u6761\u6295\u7a3f\u5df2\u79fb\u5230\u5df2\u5220\u9664\u5217\u8868\uff0c${getDeleteCountdown(item.deletedAt)}\u540e\u5c06\u6c38\u4e45\u5220\u9664\uff0c\u671f\u95f4\u4ecd\u53ef\u6062\u590d\u3002` : status.message}</p>
        ...
        <div className="mt-4 flex flex-wrap gap-2 border-t border-slate-100 pt-4">
          {deleted ? (
            ...
            <>
              {canEdit ? <Link href={`/my-submissions/${item.id}/edit`} className="inline-flex items-center gap-1 rounded-lg border border-emerald-200 bg-white px-3 py-2 text-sm font-medium text-emerald-700"><Pencil className="h-4 w-4" />{"\u4fee\u6539\u6295\u7a3f"}</Link> : null}
      ...
      <section className="mx-auto max-w-4xl px-4 py-6 md:px-6">
        ...
        {!loading && currentUser.isAuthenticated && activeSubmissions.length === 0 ? (
          <div className="mt-5 rounded-xl border border-slate-200 bg-white p-5 text-sm text-slate-600">
            ...
            <Link href="/submit-spot" className="mt-3 inline-flex rounded-lg bg-emerald-600 px-4 py-2 text-sm font-semibold text-white">{"\u53bb\u63a8\u8350\u5730\u70b9"}</Link>
          </div>
        ) : null}
