...
const selectFields =
  "id,external_id,name,name_zh,city,city_zh,latitude,longitude,scenario,distance_km,difficulty,safety,rating,has_parking,has_toilet,min_kid_age,image,description,description_zh,updated_at";
...
export async function getAdminDestinations(query = "") {
  ...
  if (keyword) {
    request = request.or(`name.ilike.%${keyword}%,name_zh.ilike.%${keyword}%,city.ilike.%${keyword}%,city_zh.ilike.%${keyword}%`);
  }
